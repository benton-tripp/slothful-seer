...
map.data <- function(df, 
  ...
  # Check original CRS and extent
  original_crs <- raster::crs(overlay) %>% as.character()
  original_extent <- raster::extent(overlay) %>% as.character()
  cat("Original CRS:", original_crs, "\n")
  cat("Original extent:", original_extent, "\n")
  
  # Reproject raster
  reprojected_overlay <- raster::projectRaster(overlay, crs = CRS("+init=epsg:3857"))
  
  # Check CRS and extent after reprojection
  reprojected_crs <- raster::crs(reprojected_overlay) %>% as.character()
  reprojected_extent <- raster::extent(reprojected_overlay) %>% as.character()
  cat("Reprojected CRS:", reprojected_crs, "\n")
  cat("Reprojected extent:", reprojected_extent, "\n")
  
  
  
  # Transform the overlay to EPSG:3857
  overlay <- projectRaster(overlay, crs = CRS("+init=epsg:3857"))

  
  # Get finite values range
  finite_values <- raster::values(resampled)[is.finite(raster::values(resampled))]
  ...
  leaflet() %>%
    ...
    hideGroup("Absence") %>%
    addControl(html = paste("<h4>", title, "</h4>"), position = "bottomleft") %>%
...
map.data(df, rasters, .raster=8, title="Three-Toed-Sloths")
...
In this context, the inhomogeneous Poisson process model (IPP) is defined as:
...
In the case of the IPP model, the intensity \(\lambda\) is not a constant, but is a function of the location, \(x\). Hence the term 'inhomogeneous'. 

In the code snippet: `prob.at.least.1 <- calc(predicted.raster, function(x) {1 - exp(-x)})`
...
if (.model.type %in% c("glm", "glmnet", "knn")) {
  ...
  # Generate predictions on rasters (to plot probabilities)
  preds.raster <- raster::predict(object=rasters, model=fit, type="prob")
